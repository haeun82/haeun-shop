...
export const getMoreProductsAsync = createAsyncThunk(
  ...
  async () => { // 두번째 인자값: action이 발생했을 때 실행할 비동기 작업(api 요청 같은)
    const result = await getMoreProducts(); // 해당 비동기 함수 실행 시 'pending' 상태가 됨
    return result; // 값을 반환하면 'fulfilled'로 상태로 바뀌고 action.payload에서 담겨 리듀서 함수로 전달됨
  }
);
...
const prodcutSlice = createSlice({
  ...
  // thunk를 이용한 비동기적인 작업에는 extraReducers 사용 
...
export const { 
  getAllProduct, 
  getSelectedProduct, 
  clearselectedProduct, addMoreProducts
} = prodcutSlice.actions;
